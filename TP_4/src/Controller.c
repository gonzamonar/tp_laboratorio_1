...
int controller_exit(LinkedList* pArrayListPassenger, LinkedList* pBackupList, int* selection, int* saveCsvFlag, int* saveBinFlag){
	char confirmation;
	int outcome = FAILURE;
	system("cls");
	...
	if(confirmation == 'S'){
		*selection = 12;
		...
		ll_deleteLinkedList(pBackupList);
		printf("\n  Ejecución concluida.");
		outcome = SUCCESS;
	}
	return outcome;
