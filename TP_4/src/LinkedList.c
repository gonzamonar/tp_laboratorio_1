...
int ll_remove(LinkedList* this, int index)
{
    ...
    if(this != NULL && index >= 0 && index < ll_len(this))
    {
    	...
    	{
    		nodeDelantero = getNode(this, index-1);
    		nodeDelantero->pNextNode = nodeAux->pNextNode;
    	}
    	this->size--;
    	free(nodeAux);
    	returnAux = 0;
 ...
 * \return int Retorna  (-1) Error: si el puntero a la lista es NULL o falla la búsqueda.
...
int ll_isEmpty(LinkedList* this)
{
    int returnAux = -1;
    if(this != NULL){
    	returnAux = (this->pFirstNode == NULL);
    }
    return returnAux;
}
    ...
    if(this != NULL && pFunc != NULL && (order == 0 || order == 1))
    {
    	...
    	for(int i=0; i<len-1; i++)
    	{
    		for(int j=i+1; j<len; j++)
    		{
    			...
    			if((order && pFunc(elementI,elementJ)>0)
    			  || (!order && pFunc(elementI,elementJ)<0))
    			{
    				buffer = elementI;
